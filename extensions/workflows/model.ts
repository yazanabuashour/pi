...
export interface AgentRecord {
  ...
  preview: string;
  usage: AgentUsage;
  /** Normalized, serializable workflow agent conversation shown by /workflows. */
...
/** Show process-owned work as interrupted without mutating a possibly live artifact. */
export function displayInterruptedWorkflow(
  ...
  return {
    ...
    error:
      details.error ??
      "Workflow is not active in this Pi runtime; rerun it instead of resuming it.",
    agents: details.agents.map((agent) =>
      agent.state === "running"
        ? {
            ...agent,
            state: "error",
            error: agent.error ?? "Agent is not active in this Pi runtime",
            finishedAt,
          }
        : agent,
    ),
...
export function resultJson<Input1>(value: Input1): string {
