...
function abortError(signal: AbortSignal) {
  return signal.reason instanceof Error
    ? signal.reason
    : new Error(String(signal.reason ?? "Workflow was aborted"), {
        cause: signal.reason,
      });
}
...
export class RunController {
  ...
  // AbortSignal reasons are arbitrary values; preserve the caller's identity.
  // oxlint-disable-next-line project/no-unknown-parameters
  abort(reason: unknown = new Error("Workflow was aborted")) {
